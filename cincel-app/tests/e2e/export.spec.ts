/**
 * E2E: Export — triggers an Excel export from Clientes and verifies a file
...
test.describe("Exportación — clientes module", () => {
  test("Exportar ▼ button appears and triggers a download when clicked", async ({ page }) => {
    ...
    await page.waitForFunction(
      () => Array.from(document.querySelectorAll("h1")).some((el) => el.textContent?.includes("Clientes")),
      { timeout: 30_000 }
    );
