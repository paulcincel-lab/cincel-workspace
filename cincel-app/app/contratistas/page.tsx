...
const statusStyle = (s: string) => {
  const map: Record<string, string> = {
    ...
    "Pausado": "bg-orange-400 text-white",
    "Lista Negra": "bg-red-600 text-white",
    "Sin actividad con nosotros": "bg-gray-400 text-white",
...
const seniorityStyle = (s: string) => {
  const map: Record<string, string> = {
    ...
    "Nivel Medio": "bg-yellow-400 text-gray-900",
    "Con detalles": "bg-yellow-500 text-white",
    "Bajo": "bg-red-400 text-white",
    "No trabajes con el": "bg-red-700 text-white",
  };
  return map[s] ?? "bg-gray-200 text-gray-700";
};
...
const priceStyle = (p: string) => {
  const map: Record<string, string> = {
    "Gama Alta": "bg-emerald-500 text-white",
    "Nivel Medio": "bg-yellow-400 text-gray-900",
    "Medio-Bajo": "bg-orange-400 text-white",
    "Bajo": "bg-red-400 text-white",
    "No Trabajes con el": "bg-red-700 text-white",
  };
...
const specialtyStyle = (s: string) => {
  const map: Record<string, string> = {
    "Albañilería": "bg-slate-800 text-white",
    "IE": "bg-slate-900 text-white",
    "Cancelería": "bg-cyan-400 text-gray-900",
    "Carpintería": "bg-amber-700 text-white",
    "Acústica": "bg-orange-300 text-gray-900",
    "Tapicería": "bg-purple-300 text-gray-900",
    "Colocación": "bg-stone-700 text-white",
    "Herrería": "bg-gray-500 text-white",
    "Pintura": "bg-slate-400 text-white",
    "Limpieza": "bg-gray-300 text-gray-700",
    "Herrero": "bg-gray-500 text-white",
    "General": "bg-gray-300 text-gray-700",
    "IHS": "bg-indigo-600 text-white",
    "HVAC": "bg-sky-600 text-white",
    "Mármoles": "bg-stone-400 text-white",
    "Pisos": "bg-amber-500 text-white",
    "Paneles": "bg-teal-500 text-white",
    "Colocación Pisos Madera": "bg-stone-700 text-white",
    "Colocación y Acabados": "bg-stone-600 text-white",
    "Mobiliario Habitacional": "bg-blue-400 text-white",
    "Mob Restaurante": "bg-blue-500 text-white",
    "Mob Oficina": "bg-blue-600 text-white",
    "Espejos": "bg-cyan-300 text-gray-900",
    "Iluminación": "bg-yellow-300 text-gray-900",
    "Instalaciones Especiales": "bg-indigo-500 text-white",
    "Soluciones de Ingeniería": "bg-violet-600 text-white",
    "Materiales de Construcción": "bg-stone-500 text-white",
    "Macetas": "bg-lime-500 text-white",
  };
  ...
  return (
    <div className="relative inline-block">
      <button
        ...
        className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap cursor-pointer hover:opacity-80 transition ${pillStyle.className ?? ""}`}
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 z-50 min-w-[220px] bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {COLOR_PALETTE.map((c) => (
                    <button
                      ...
                      className="w-8 h-8 rounded-full border-2 border-white hover:border-gray-400 transition shadow-sm"
                  ...
                  {options.map((opt) => {
                    ...
                    return (
                      <div key={opt} className={`flex items-center group px-2 py-1.5 hover:bg-gray-50 ${value === opt ? "bg-gray-50" : ""}`}>
                        ...
                        >
                          <span
                            className={`w-3 h-3 rounded-full flex-shrink-0 ${s.className ? s.className.split(" ")[0] : ""}`}
